...
const unsigned char Firmware[2] = {1,01};	// {Major, Minor}
const unsigned char DriverIC = {0};             // Driver IC Type: 1 - HV892
    ...
    while (1)
    {
        ...
        if(rxd==1)                  // check for received data
        {
            switch(rx_data.command)
            {
                ...
                case SET_VOLT:
                    // send I2C commands to send voltage
                    Amplitude = rx_data.data[0];
                    I2C_Start();
