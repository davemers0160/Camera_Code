...
% Example Usage of Function: [texturelessImg, timeTaken]=funcTexturelessRegions('view5_color.tif',5, 4);
% *************************************************************************
...
try
    ...
    %inputImage=imread('view5_color.tif');
    %inputImage=imread('Cbin.png');
    inputImage=imread('Crin.png');
    windowSize=9;
    thresh=4;
...
for col=1:win
    for row= 1:nr
        texturelessImg(row,col)=texturelessImg(row,win+1);
    end
end

for row=1:win
    for col= 1:nc
        texturelessImg(row,col)=texturelessImg(win+1,col);
    end
end

for col=nc-win+1:nc
    for row= 1:nr
        texturelessImg(row,col)=texturelessImg(row,nc-win);
    end
end

for row=nr-win:nr
    for col= 1:nc
        texturelessImg(row,col)=texturelessImg(nr-win,col);
    end
end
% Stop the timer to calculate the time consumed.
timeTaken=toc;
%imwrite(texturelessImg,'texturelessregion.png');
imwrite(texturelessImg,'texturelessregionCb.png');
imwrite(texturelessImg,'texturelessregionCr.png');

imshow(texturelessImg);